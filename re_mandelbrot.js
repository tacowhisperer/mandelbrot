...
function readSettingsHandler (err, settings) {
	...
	// Manipulate the save settings buffer as specified
	else {
...
function createNewSettings () {
	...
	if (answer.match (/^y/i)) {
		...
		fs.open ('./' + SETTINGS_FILE, 'w', function (err, fd) {
			if (err) {
				saveBuffer = null;
				console.log ('There was an error creating the settings file'.red);
				console.log (('' + err).red);
			}

			else {
				// Merge the file data into the save Buffer Array
				var sBuff = saveBuffer.mergeBuffer (SAVE_FILE_SIGNATURE)
				...
				fs.write (fd, sBuff, 0, sBuff.length, function (e) {
					if (e) {
						console.log ('There was an error writing the settings file'.red);
						console.log (('' + e).red);
					}
		...
		fs.open ('./' + SETTINGS_FILE, 'w', function (err, fd) {
			if (err) {
				console.log ('There was an error creating a reduced save file.'.red);
				console.log (('' + err).red);
			}

			else {
				var sBuff = saveBuffer.mergeBuffer (SAVE_FILE_SIGNATURE)
									  .mergeBuffer (new Buffer ([0]))
									  .buffer ();
				fs.write (fd, sBuff, 0, sBuff.length, function (e) {
					if (e) {
						console.log ('There was an error writing the reduced save file'.red);
						console.log (('' + e).red);
					}
